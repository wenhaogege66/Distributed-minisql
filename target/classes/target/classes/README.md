...
3. **备份机制**：当存在 3 个以上的 RegionServer 时，系统会保留至少 1 个 RegionServer 用于备份
...
2. **自动恢复**：当检测到 RegionServer 故障时，会自动将其负责的数据迁移到其他健康的 RegionServer
3. **数据复制**：每个表默认有 2 个副本，确保单点故障不会导致数据丢失
4. **优雅降级**：即使部分 RegionServer 不可用，系统仍然能够提供服务
