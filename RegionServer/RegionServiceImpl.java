            ...
            for (Map<String, Object> row : data) {
                ...
                for (Map.Entry<String, Object> condition : conditions.entrySet()) {
                    ...
                    if (columnName.endsWith("_range") && value instanceof Map) {
                        ...
                        // 去掉尾部的 "=" 得到实际的列名
                        String col = columnName.substring(0, columnName.length() - 1);
                        if (!row.containsKey(col) || !objectsEqual(row.get(col), value)) {
                            match = false;
                            break;
                        }
                    }
                    // 对于 "大于" 的情况：
                        ...
                        // 此处要求 row.get(col) 和 value 都是 Comparable 类型
                        Comparable fieldValue = (Comparable) row.get(col);
